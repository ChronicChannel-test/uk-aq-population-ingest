...
- Dependencies: SUPABASE_URL, SB_SECRET_KEY (SB_SUPABASE_URL is an optional URL fallback); expects REST access to view uk_population_observations.
...
- Environment: SUPABASE_URL, SB_SECRET_KEY, NOMIS_BASE_URL (default https://www.nomisweb.co.uk/api/v01), NOMIS_DATASET_IDS (comma-separated list of tracked ids).
...
- Environment: SUPABASE_URL, SB_SECRET_KEY, UK_POPULATION_CATALOGUE_URL, UK_POPULATION_CATALOGUE_REPLACE (boolean, default true).
...
- Environment: SUPABASE_URL, SB_SECRET_KEY, UK_POPULATION_INGEST_CONFIG_URL or UK_POPULATION_INGEST_CONFIG (JSON), UK_POPULATION_PREFIXES (default nrs,nisra), UK_POPULATION_BATCH_SIZE (default 500).
